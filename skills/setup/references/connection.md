...
- After the user returns, read status once and rediscover tools. Status resumes a pending request or saved conversation authorization after a helper restart when the same plugin data store is available; it does not start new consent. Resume the original request in this conversation. If data tools are still unavailable, report the actual state. A hosted “Sign-in received” page alone does not establish access. Do not infer a restart or lost authorization from `idle` alone; use the returned non-secret `plugin_version`, `helper_instance` and `connection_route` when diagnosing a failure.
- **`failed`:** retry once with `adzviser_sign_in`. It resumes a still-valid request or returns a new link if the old one expired. Do not ask the user to repeat consent unless the returned result requires it. If it fails again, report the non-secret error and use the host's supported connection controls as recovery. Do not switch accounts or repeat sign-ins indefinitely.
