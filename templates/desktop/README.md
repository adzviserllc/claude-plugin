...
1. In **Customize → Plugins**, refresh the marketplace from `https://github.com/adzviserllc/claude-plugin`. Install **adzviser-desktop**, version **0.1.0-rc.3**, in an ordinary working folder. If the original **adzviser** plugin is installed, turn it off for this test to avoid duplicate skill names. Keep the directory connector disabled during this independence test.
...
Version **0.1.0-rc.3** adds an Adzviser-branded callback page with a clear return-to-Desktop message. The localhost address receives the sign-in result on your computer; the page clears the code and state from the address bar and makes no external resource requests. The page gives browser-tab close instructions and keyboard shortcuts; it does not offer a close button that browsers may block. The page acknowledges receipt of the authorization response; a successful tool call in Claude confirms account access.
