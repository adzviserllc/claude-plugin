...
Resolve the requested account and metric definitions before comparing sources. Start with totals, then request detail only to investigate a material question. Use the Adzviser MCP tools available in the session; if they are unavailable, return that limitation instead of simulating tool output.
