...
**Adzviser 1.3.1** is a conversation sign-in preview. The hosted callback service is deployed and enabled. This update fixes pending sign-in recovery after helper restarts; automated plugin/backend integration covers shutdown, crashes and shared requests. Actual Cowork sign-in, persistence and reporting remain required before submission as supported. Existing Linux local Code workspace/reporting evidence is preserved in the [release checks](docs/testing.md).
...
| Publisher | Adzviser |
| Version | `1.3.1` (conversation sign-in preview) |
...
The plugin bundles a remote HTTP connection managed by Claude and a local Node.js connection with its own sign-in. Local Code does not require the directory connector. For remote access, Claude may match the bundled URL to the existing Adzviser directory entry; do not claim that this entry remains disabled or that its credentials are separate. The local helper runs with the user's OS permissions. Cowork, Chat, web, mobile, remote Code, macOS, and Windows need actual app acceptance tests before support claims.
