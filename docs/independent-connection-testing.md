...
The **adzviser-desktop 0.1.0-rc.3** candidate uses a plugin-declared local stdio connection with `mcp-remote@0.14.2`, an independent server name, and an isolated OAuth cache. Its workflows require tools from that connection; they must not silently fall back to the directory connector. The callback presentation adapter leaves the helper's OAuth implementation unchanged. Revision rc.3 replaces the unreliable tab-close button with manual-close instructions and keyboard shortcuts.
