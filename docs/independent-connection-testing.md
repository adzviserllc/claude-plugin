...
The **adzviser-desktop 0.1.0-rc.1** candidate uses a plugin-declared local stdio connection with `mcp-remote@0.14.2`, an independent server name, and an isolated OAuth cache. Its workflows require tools from that connection; they must not silently fall back to the directory connector.
